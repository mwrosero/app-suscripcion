                                                ...
                                                <div class="col-md-12 col-xl-8">
                                                    <div class="form-check">
                                                        ...
                                                        <label class="form-check-label fs-10p">
                                                            He leído y comprendo la autorización para el <a href="https://www.veris.com.ec/politicas/" target="_blank" class="text-mariner-600 text-decoration-underline" nemonico-rel="TRATAMIENTO_DATOS">Tratamiento de mis datos personales</a> <span class="text-danger">*</span>
                                                        </label>
                                                    </div>
                                                </div>
                                                <hr>
...
<script>
    ...
    async function renderButtons(idx) {
        ...
        if (idx === 0) {
            ...
            if(detalleSuscripcion.hasOwnProperty('tarjeta')){
                ...
                $('#btn-next').removeClass('btn-firmar-documentos').removeClass('btn-generar-solicitud').removeClass('btn-continuar-tipo-diferido').addClass('btn-suscribir-tarjeta');
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        $('body').on('click', '.nav-metodo-pago', function () {
            ...
            if ($(this).find('.nav-link').attr('id') === 'pills-credit-card-tab') {
                console.log("Bloquear botón continuar");
                $('#btn-next').attr('disabled', false);
                esTarjetaBox = true;
            }else{
        ...
        $('body').on('click', '.btn-continuar-tipo-diferido', async function(){
            ...
            $('#btn-next').attr('disabled', true);
            stepper.next();
            $('#btn-next').removeClass('btn-continuar-tipo-diferido').addClass('btn-firmar-documentos');
        })
    ...
    $("#add-card-form").submit(function (e) {
        ...
        $('#messages').text("");
        let cardToSave = myCard.PaymentForm('card');
        console.log('cardToSave #add-card-form');
        if (cardToSave == null) {
            hideLoader();
            $('#messages').text("Datos inválidos de tarjeta, verifique la información ingresada.");
        }else{
    ...
    let successHandler = async function (cardResponse) {
        ...
        if (cardResponse.card.status === 'valid') {
            console.log("-----SUSCRITO, LLAMAR DIFERIDOS-----")
    ...
    async function obtenerListadoDocumentosFirma(){
        let args = [];
        args["endpoint"] = api_url + `/${api_war_empresarial}/v1/util/suscripcion/tipos_documentos?estado=ACTIVO&aplicaFirma=true&flujoSuscripcion=COLABORADOR`;
        args["method"] = "GET";
    ...
    async function obtenerDocumentoContrato(datos){
        ...
        if(datos.nemonico == "AUTORIZACION_DEBITO"){
            ...
            args["endpoint"] = api_url + `/${api_war_empresarial}/v1/suscripcion/documentos_firma?codigoEmpresa=1&nemonicoDocumento=${datos.nemonico}&flujoSuscripcion=C`;
            args["method"] = "POST";
            args["bodyType"] = "json";
    ...
    async function cargarMediosPago(){
        let args = [];
        args["endpoint"] = `${api_url}/${api_war_empresarial}/v1/util/suscripcion/medios_pago?estado=ACTIVO&flujoSuscripcion=COLABORADOR&codigoCliente=${detalleSuscripcion.empresa.codigoEmpresa}`
        args["method"] = "GET";
        args["showLoader"] = true;
        args["token"] = _token;
        const data = await call(args);
    ...
    async function detallesAprobacionDescuentoRol(){
        ...
        let args = [];
        args["endpoint"] = `${api_url}/${api_war_empresarial}/v1/util/suscripcion/${codigoPersonal}/aprobacion_descuento_rol?codigoEmpresa=1&secuenciaFrecuencia=${detalleSuscripcion.detallePlan.secuenciaFrecuencia}`
        args["method"] = "GET";
        args["showLoader"] = true;
        args["token"] = _token;
        const data = await call(args);
